#!/usr/bin/env python3
"""
Test g3icap with Google HTTPS request
This script sends a proper ICAP REQMOD request with Google's HTTPS request data
"""

import socket
import time

def test_google_icap():
    print("🌐 Testing g3icap with Google HTTPS Request")
    print("=" * 60)
    ...
    try:
        ...
        # Create Google HTTPS request data
        google_https_request = (
            b"GET / HTTP/1.1\r\n"
        ...
        req_hdr_len = len(google_https_request)
        req_body_len = 0  # No body for GET request
        
        # Create ICAP REQMOD request with Google's HTTPS data
        icap_request = (
        ...
        print("📤 Sending Google HTTPS REQMOD request:")
        print("-" * 60)
        ...
        if response:
            print("📥 Received Google HTTPS REQMOD response:")
            print("=" * 60)
            ...
            if "server:" in response_str and "istag:" in response_str:
                ...
                if "204 No Content" in response_str:
                    print("✅ Google HTTPS request allowed (204 No Content)")
                elif "403 Forbidden" in response_str:
                    print("❌ Google HTTPS request blocked (403 Forbidden)")
                elif "200 OK" in response_str:
                    print("✅ Google HTTPS request modified (200 OK)")
                else:
                    print(f"ℹ️  Google HTTPS request status: {response_str.split()[1] if len(response_str.split()) > 1 else 'Unknown'}")
            else:
        ...
        print(f"❌ Error testing Google HTTPS request: {e}")
    
    print("\n" + "=" * 60)
    print("✅ Google HTTPS ICAP test completed!")
